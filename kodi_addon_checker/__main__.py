...
from kodi_addon_checker import __version__, ValidKodiVersions
...
def dir_type(dir_path):
    ...
    if not os.path.isdir(dir_path):
        raise argparse.ArgumentTypeError(f"Add-on directory {dir_path} does not exist")
...
def main():
    ...
    parser.add_argument(
        "--allow-folder-id-mismatch", help="Allow the addon's folder name and id to mismatch", action="store_true"
    )
    parser.add_argument(
        "--enable-debug-log", help="Enable debug logging to kodi-addon-checker.log", action="store_true", default=False
    )
    parser.add_argument(
        "--skip-dependency-checks",
        help="Do not check if addon dependencies are available \
                        in the official Kodi addon repository",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--addon-repo-domain", help="Domain for addon repository (default: mirrors.kodi.tv)", default="mirrors.kodi.tv"
    )
    ...
    all_repo_addons = check_addon.get_all_repo_addons(args.addon_repo_domain)
    print(f"Loaded {len(all_repo_addons)} repo addons")
    ...
    if report.problem_count > 0:
        report.add(
            Record(
                PROBLEM,
                f"We found {report.problem_count} problems "
        ...
        report.add(
            Record(WARNING, "We found no problems " f"and {report.warning_count} warnings, please check the logfile.")
        )
    else:
